...
static void update_button(unsigned int id, bool &button, bool *old_val = nullptr) {
	bool new_val = input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, id) != 0;
	if (old_val == nullptr || new_val != *old_val) {
		button = new_val;
		if (old_val) {
			*old_val = new_val;
		}
...
static void update_input() {
	...
	update_button(RETRO_DEVICE_ID_JOYPAD_X, pi.action);
...
void retro_run(void) {
	...
	uint16_t samplesPerFrame = game->getOutputSampleRate() / game->getFrameRate();
