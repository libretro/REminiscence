...
static INLINE int8_t ADDC_S8(int a, int b) {
	a += b;
	if (a < -128)
		a = -128;
	else if (a > 127)
...
static INLINE int16_t ADDC_S16(int a, int b) {
	a += b;
	if (a < -32768)
		a = -32768;
	else if (a > 32767)
...
struct LivePGE {
	...
	struct LivePGE  *next_PGE_in_room;
	struct InitPGE  *init_PGE;
};

struct GroupPGE {
	struct GroupPGE *next_entry;
	uint16_t index;
	uint16_t group_id;
};
...
struct CollisionSlot {
	int16_t       ct_pos;
	struct CollisionSlot *prev_slot;
	struct LivePGE       *live_pge;
	uint16_t      index;
};
...
struct CollisionSlot2 {
	struct CollisionSlot2 *next_slot;
	int8_t         *unk2;
	uint8_t        data_size;
